...
int generate(char *input1, char *input2, char *output, char *fileName)
{
...
int main (int argc, char **argv)
{
	...
	int opt;
	int silent = 0;
	while ((opt = getopt (argc, argv, "1:2:o:hvs")) != -1)
	{	
		switch (opt)
		{
		case '1':
	...
	if (binded == 1)
	{
		...
		if (exists(input1) == 0 || exists(input2) == 0)
		{
			if (silent == 0)
			{
				printf("  One or more input files do not exist.\n  Please try again.");
			}
			return 1;
		}
		...
		generate(input1, input2, output, argv[0]);
		if (silent == 0)
		{
